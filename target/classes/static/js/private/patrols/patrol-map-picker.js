import{f as y}from"../../auth.js";import{n as f}from"../../navigation-handler.js";let l=[],s=[],d=null,c=null;const h=t=>document.querySelector(t),b=(()=>{let t=!1,e=null;return o=>(t||(e=new Promise((n,a)=>{const i=document.createElement("script");i.src=`https://maps.googleapis.com/maps/api/js?key=${o}&v=weekly`,i.async=!0,i.defer=!0,i.onload=()=>{console.log("[Google Maps API] Script cargado correctamente."),t=!0,n()},i.onerror=r=>{console.error("[Google Maps API] Error al cargar el script:",r),a(new Error("Error al cargar Google Maps API"))},document.head.appendChild(i)})),e)})(),w=async()=>{const t=document.getElementById("patrols-map-picker");if(!t){console.warn("[initMap] Contenedor #patrols-map-picker no encontrado en el DOM.");return}console.log("siteId en initMap: "+window.targetSiteId);try{const{Map:e}=await google.maps.importLibrary("maps"),o=new e(t,{center:{lat:-33.4489,lng:-70.6693},zoom:8,mapId:googleMapsConfig.mapId,disableDefaultUI:!0,mapTypeId:"hybrid"});console.log("[initMap] Mapa inicializado."),window.mapInstance=o,await k(),await $(),o.addListener("click",n=>{v(n.latLng)})}catch(e){console.error("[initMap] Error al inicializar el mapa:",e)}};async function k(){const t=document.getElementById("target-site-id"),e=t?t.value:null;if(!e){console.error("No se pudo obtener el ID del sitio del input oculto");return}try{const o=await y("/api/sites/projections-by-user",{method:"GET",headers:{Accept:"application/json"}});if(!o.ok){console.log("Error al cargar sitios. Intente nuevamente.");return}const n=await o.json();if(!Array.isArray(n)||!n.every(i=>i.id&&i.lat&&i.lon)){console.log("Datos de sitios inválidos.");return}console.log("Buscando siteId:",e),console.log("IDs disponibles en siteData:",n.map(i=>i.id));const a=n.find(i=>Number(i.id)===Number(e));console.log("site: "+a),x(a)}catch(o){console.error("Fallo al obtener sitios:",o),console.log("Error al cargar sitios. Intente nuevamente.")}}async function x(t){const{AdvancedMarkerElement:e,PinElement:o}=await google.maps.importLibrary("marker"),n=new google.maps.LatLngBounds,a=document.createElement("div");a.style.backgroundColor="#fff",a.style.border="1px solid grey",a.style.padding="4px 8px",a.style.borderRadius="8px",a.style.boxShadow="0 2px 6px rgba(0,0,0,0.3)",a.style.position="absolute",a.style.top="-40px",a.style.left="50%",a.style.transform="translateX(-50%)",a.style.whiteSpace="nowrap",a.style.display="none",a.textContent=`Sitio: ${t.name||"Sin Nombre"}`;const i=new o({scale:.8,glyphColor:"#3176e3",background:"#359dd1",borderColor:"#1d4d9b"}),r=document.createElement("div");r.style.position="relative",r.appendChild(i.element),r.appendChild(a),new e({map:window.mapInstance,position:{lat:t.lat,lng:t.lon},content:r}),r.addEventListener("mouseenter",()=>{a.style.display="block"}),r.addEventListener("mouseleave",()=>{a.style.display="none"}),n.extend(new google.maps.LatLng(t.lat,t.lon)),console.log(`[addSitesToMapAndSelect] Marcador añadido para sitio: ${t.id}`),window.mapInstance.fitBounds(n)}async function m(t,e){const{InfoWindow:o}=await google.maps.importLibrary("maps"),n=l[e];if(!n){console.error("No se encontró el punto para el índice:",e);return}const a=e===0?"display:none;":"display:block;";c&&c.close(),c=new o({content:`
            ...
            </tr>`;t.insertAdjacentHTML("beforeend",a)})}function I(){document.getElementById("btn-clear-path")?.addEventListener("click",()=>{confirm("¿Borrar todos los puntos?")&&C()}),h("#btn-confirm-map").addEventListener("click",M)}window.removeCheckpoint=function(t){s[t]&&s[t].setMap(null),l.splice(t,1),s.splice(t,1),l.forEach((e,o)=>e.order=o+1),E(),u(),p(),c&&c.close()};function E(){s.forEach((t,e)=>{const o=e+1,n=new google.maps.marker.PinElement({glyphText:o.toString(),background:"#FBBC04"});t.content=n.element,t.title=`Punto ${o}`,google.maps.event.clearInstanceListeners(t),t.addListener("click",()=>{m(t,e)})})}function u(){const t=l.map(e=>({lat:e.lat,lng:e.lng}));d?d.setPath(t):d=new google.maps.Polyline({path:t,geodesic:!0,strokeColor:"#FF0000",strokeOpacity:1,strokeWeight:3,map:window.mapInstance})}function C(){s.forEach(t=>t.setMap(null)),s=[],l=[],d&&d.setPath([]),p(),c&&c.close(),console.log("Ruta reseteada correctamente.")}async function M(){if(l.length===0){alert("Define al menos un punto en la ruta antes de confirmar.");return}localStorage.setItem("pending_checkpoints",JSON.stringify(l));const e=`/private/patrols/edit/${document.getElementById("target-patrol-externalId").value}`;console.log(`[MapPicker] Finalizando edición. Navegando a ${e}`),await f(e)}const $=async()=>{const{AdvancedMarkerElement:t,PinElement:e}=await google.maps.importLibrary("marker"),o=window.checkpoints||[];if(o.length!==0){console.log(`[loadExistingCheckpoints] Dibujando ${o.length} puntos...`);for(const n of o){const a={lat:parseFloat(n.latitude),lng:parseFloat(n.longitude)},i=new e({glyph:n.checkpointOrder.toString(),background:"#FBBC04",borderColor:"#137333",glyphColor:"white"}),r=new t({map:window.mapInstance,position:a,content:i.element,title:n.name});l.push({externalId:n.externalId,lat:a.lat,lng:a.lng,order:n.checkpointOrder,name:n.name,stayTime:n.stayTime,transitTime:n.minutesToReach}),s.push(r),r.addListener("click",()=>{const g=s.indexOf(r);g!==-1&&m(r,g)})}p(),u(),window.mapInstance.setCenter({lat:parseFloat(o[0].latitude),lng:parseFloat(o[0].longitude)}),window.mapInstance.setZoom(16)}};(async function(){console.log("[init] IIFE iniciado");const e=googleMapsConfig.apiKey;try{await b(e),w(),I()}catch(o){console.error("[site-map.js] Error al cargar la API de Google Maps:",o)}})();
