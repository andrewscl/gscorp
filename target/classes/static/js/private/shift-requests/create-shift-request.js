import{f as R}from"../../auth.js";import{n as L}from"../../navigation-handler.js";import{d as l}from"../../shared/display-alert.js";import"../../shared/dom-utils.js";const i=(t,e=document)=>e.querySelector(t),y=(t,e=document)=>Array.from((e||document).querySelectorAll(t)),b={Lunes:0,Martes:1,Miércoles:2,Jueves:3,Viernes:4,Sábado:5,Domingo:6},k=Object.keys(b);function D(t=document.querySelector("#shiftDayRanges")){y(".day-range-block",t).forEach((n,r)=>{y("input, select, textarea",n).forEach(a=>{const o=a.getAttribute("name");if(!o)return;const s=o.replace(/schedules\[\d+\]/,`schedules[${r}]`);a.setAttribute("name",s)})})}function w(t){const e=t.map(n=>{let r=b[n.dayFrom],a=b[n.dayTo];return r>a&&(a+=7),{from:r,to:a,orig:n}});for(let n=0;n<e.length;n++)for(let r=n+1;r<e.length;r++){const a=e[n],o=e[r];for(let s=a.from;s<=a.to;s++)for(let u=o.from;u<=o.to;u++)if(s%7===u%7)return[a.orig,o.orig]}return null}async function H(){const t=i("#submit"),e=i("#cancel"),n=i("#shiftRequestSite")?.value,r=i("#shiftRequestAccount")?.value,a=i("#shiftRequestServiceType")?.value,o=i("#shiftRequestStartDate")?.value,s=i("#shiftRequestEndDate")?.value||null,u=i("#shiftRequestDescription")?.value?.trim()||null,T=n?parseInt(n,10):null,A=r?parseInt(r,10):null;if(!T){l(alertError,"Debe seleccionar un sitio.");return}if(!a){l(alertError,"Debe seleccionar el tipo de servicio.");return}if(!o){l(alertError,"La fecha de inicio es obligatoria.");return}const p=[];if(y(".day-range-block").forEach((c,m)=>{const f=c.querySelector(".dayFrom")?.value,v=c.querySelector(".dayTo")?.value,S=c.querySelector('input[name$="[startTime]"]')?.value,q=c.querySelector('input[name$="[endTime]"]')?.value;f&&v&&S&&q&&p.push({dayFrom:f,dayTo:v,startTime:S,endTime:q})}),p.length===0){l(alertError,"Debe ingresar al menos un tramo de horario.");return}const d=w(p);if(d){l(alertError,`Solapamiento de días entre "${d[0].dayFrom}
