...
public interface ShiftRequestScheduleRepository extends JpaRepository<ShiftRequestSchedule, Long> {
    ...
    @Query("""
      SELECT sc.id                AS id,
             ...
             sc.startTime         AS startTime,
             sc.endTime           AS endTime,
             sr.startDate         AS requestStartDate,
             sr.endDate           AS requestEndDate
      FROM ShiftRequestSchedule sc
      JOIN sc.shiftRequest sr
    ...
    @Query("""
      ...
      FROM ShiftRequestSchedule sc
      JOIN sc.shiftRequest sr
      WHERE sc.shiftRequest.id = :id
    """)
    List<ShiftRequestScheduleProjection> findByShiftRequestId(@Param("id") Long id);
    ...
    List<ProjectSiteShiftRequestSchedulesProjection>
                  findProjectSiteShiftRequestSchedulesByClientIds(
