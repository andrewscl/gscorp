import{f as R}from"../../auth.js";import{n as L}from"../../navigation-handler.js";import{d as l}from"../../shared/display-alert.js";import"../../shared/dom-utils.js";const i=(t,e=document)=>e.querySelector(t),y=(t,e=document)=>Array.from((e||document).querySelectorAll(t)),b={Lunes:0,Martes:1,Miércoles:2,Jueves:3,Viernes:4,Sábado:5,Domingo:6},k=Object.keys(b);function D(t=document.querySelector("#shiftDayRanges")){y(".day-range-block",t).forEach((n,o)=>{y("input, select, textarea",n).forEach(a=>{const r=a.getAttribute("name");if(!r)return;const s=r.replace(/schedules\[\d+\]/,`schedules[${o}]`);a.setAttribute("name",s)})})}function w(t){const e=t.map(n=>{let o=b[n.dayFrom],a=b[n.dayTo];return o>a&&(a+=7),{from:o,to:a,orig:n}});for(let n=0;n<e.length;n++)for(let o=n+1;o<e.length;o++){const a=e[n],r=e[o];for(let s=a.from;s<=a.to;s++)for(let u=r.from;u<=r.to;u++)if(s%7===u%7)return[a.orig,r.orig]}return null}async function H(){const t=i("#submit"),e=i("#cancel"),n=i("#shiftRequestSite")?.value,o=i("#shiftRequestAccount")?.value,a=i("#shiftRequestServiceType")?.value,r=i("#shiftRequestStartDate")?.value,s=i("#shiftRequestEndDate")?.value||null,u=i("#shiftRequestDescription")?.value?.trim()||null,T=n?parseInt(n,10):null,A=o?parseInt(o,10):null;if(!T){l(alertError,"Debe seleccionar un sitio.");return}if(!a){l(alertError,"Debe seleccionar el tipo de servicio.");return}if(!r){l(alertError,"La fecha de inicio es obligatoria.");return}const p=[];if(y(".day-range-block").forEach((c,m)=>{const f=c.querySelector(".dayFrom")?.value,h=c.querySelector(".dayTo")?.value,S=c.querySelector('input[name$="[startTime]"]')?.value,q=c.querySelector('input[name$="[endTime]"]')?.value;f&&h&&S&&q&&p.push({dayFrom:f,dayTo:h,startTime:S,endTime:q})}),p.length===0){l(alertError,"Debe ingresar al menos un tramo de horario.");return}const d=w(p);if(d){l(alertError,`Solapamiento de días entre "${d[0].dayFrom}
                              a ${d[0].dayTo}" y "${d[1].dayFrom}
                              a ${d[1].dayTo}".
                              Ajuste los tramos para que no se crucen.`);return}t&&(t.disabled=!0),e&&(e.disabled=!0);try{const c=await R("/api/shift-requests/create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({siteId:T,type:a,clientAccountId:A,startDate:r,endDate:s,description:u,schedules:p})});if(!c||!c.ok){let m="Ocurrió un problema al enviar el formulario.";if(c){const f=c.headers.get("content-type");f&&f.includes("application/json")&&(m=(await c.json()).message||m)}l(alertError,`Error: ${m}`),t&&(t.disabled=!1),e&&(e.disabled=!1);return}l(alertSuccess,"La asignación de turno ha sido creada correctamente.",2e3),setTimeout(()=>{L("/private/shift-assignments/list",!0)},2e3)}catch(c){console.error(`[onClickCreate] Ocurrio un problema: ${c.message}`,c),l(alertError,"Error inesperado. Intente más tarde.",2e3),t&&(t.disabled=!1),e&&(e.disabled=!1)}}function g(t){const e=i("#shiftDayRanges"),n=y(".day-range-block").length,o=k.map(s=>`<option value="${s}">${s}</option>`).join(""),a=k.map(s=>`<option value="${s}">${s}</option>`).join(""),r=document.createElement("div");r.className="day-range-block",r.innerHTML=`
