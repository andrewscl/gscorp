import{f as h}from"../../../auth.js";import{n as g}from"../../../navigation-handler.js";import{l as v}from"../../../shared/maps/googlemaps-loader.js";import{i as w}from"../../../shared/maps/init-map.js";import{a as E}from"../../../shared/maps/advanced-marker.js";import{d as o}from"../../../shared/display-alert.js";import{g as T}from"../../../shared/maps/map-utils.js";import"../../../shared/dom-utils.js";let r=null,n=null;const C=35,s=a=>document.querySelector(a),b=s(".alert-success"),l=s(".alert-error");s(".alert-warning");const u=s(".alert-info");async function M(){if(!navigator.geolocation){o(l,"Geolocalización no soportada.",3e3);return}o(u,"Detectando el sitio mas cercano...",3e3);try{const a=await new Promise((t,i)=>navigator.geolocation.getCurrentPosition(t,i,{enableHighAccuracy:!0,timeout:15e3}));if(!c.length){o(l,"No hay sitios configurados.",3e3);return}const e=T(a.coords.latitude,a.coords.longitude,c);e&&e.distance<=C?(o(b,`Estás en el sitio "${e.name}".
            ...
            </div>`;return}e.innerHTML=a.map((t,i)=>{const d=t.startTime?t.startTime.substring(0,5):"--:--",f=["is-scheduled","is-free","is-supervision"],y=f[i%f.length];return t.status==="En progreso"||t.status==="Completada"||t.status,`
    ...
    `}).join(""),e&&e.querySelectorAll(".patrol-action-card").forEach(t=>{t.addEventListener("click",i=>{const d=i.currentTarget.getAttribute("data-url");o(b,"Abriendo bitácora de ronda...",1500),setTimeout(()=>g(d,1500))})})}},N=()=>{setTimeout(()=>g("/private/employees/dashboard",!0),1e3)};function $(){const a=s("#backToEmployeeDashboard");a&&a.addEventListener("click",N)}async function I(){o(u,"Conectando con el servicio de Georreferenciación...",1500);try{await A(),await m(null),await M()}catch(a){console.error("[patrol-dashboard] initComponent failed",a)}}(async function(){$(),await I(),console.log("View patrol-dashboard page initialized.")})();
